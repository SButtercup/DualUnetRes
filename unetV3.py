...
for n, id_ in tqdm(enumerate(test_ids), total=len(test_ids)):
    ...
    mask_test = resize(mask, (256, 256, 1), mode = 'constant', preserve_range = True)
...
def the_unet(inputs):
    ...
    u8=tf.keras.layers.Conv2DTranspose(128,(2,2),strides=(2,2),padding='same')(c5)
...
def plot_sample(X, Y, preds, ix):
    ...
    ax[1,0].imshow(Z[ix].squeeze(),cmap='twilight')
    ax[1,0].set_title('Original Image for Prediction')
    ax[1,1].imshow(preds[2][12].squeeze(), cmap='twilight', vmin=0, vmax=1)
    #if has_mask:
